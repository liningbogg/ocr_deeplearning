...
model.add(Dropout(0.5))
model.add(Dense(elem_num, activation='softmax'))
...
history = model.fit_generator(train_gen.flow(image_train, train_target, batch_size=128), steps_per_epoch=len(image_train) / 128, epochs=100)

# model.fit(image_train, train_target, batch_size=64, epochs=200)
model.evaluate(image_val, val_target)
out_set = model.predict(image_val)
index = 0
for out in out_set:
    # print(np.around(val_target[index], decimals=1))
