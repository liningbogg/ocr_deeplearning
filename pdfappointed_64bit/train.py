...
model.fit(image_train, train_target, batch_size=64, epochs=20, validation_split=0.2)
model.evaluate(image_val, val_target)
out_set = model.predict(image_val)
model.save("data/withoutdot.h5")
index = 0
for out in out_set:
    for out_elem in out:
        print("%0.2f " % out_elem, end='')
    print('\n')
    for target_elem in val_target[index]:
        print("%0.2f " % target_elem, end='')
    print('\n')
    print('-----------------')
    index = index +1
